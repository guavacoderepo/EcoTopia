...
def create_user():
    ...
    if request.method == "POST":
        ...
        data = {"Name":name,"Email":email,"Phone":phone,"Username":username,"Balance":0.0,"Address":user_token,"Transactions":[],"Cart":[], "LastScan":00}
        # set status to true
        status = True
...
def login_user():
    ...
    if request.method == "POST": 
        ...
        # return data 
        data = {"Address": user["Address"],"Name":user["Name"],"Email":user["Email"],"Phone":user["Phone"],"Username":user["Username"], "LastScan":user["LastScan"]}
        status = True 
